    ...
    <header class="sticky top-0 z-50 bg-white border-b border-neutral-100 h-16">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          ...
          <!-- Instagram -->
          <a
            href="https://instagram.com/_athena_brand"
            target="_blank"
          ...
          @if (auth.isAuthenticated()) {
            <div class="relative group">
              <button
                class="w-8 h-8 rounded-full bg-black text-white text-xs font-black
                             flex items-center justify-center hover:bg-neutral-800 transition-colors"
              >
                {{ initials() }}
              </button>
              <div
                class="absolute right-0 top-full mt-2 w-40 bg-white rounded-xl shadow-xl
